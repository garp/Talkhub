const model = require('../models/hashtag.model');
...
exports.removeSavedHashtag = async ({ filter, session = null }) => dal.findOneAndDelete(saveModel, { filter, session });
...
exports.incrementViewCount = async ({ hashtagId, session = null }) => model.findByIdAndUpdate(
  hashtagId,
  { $inc: { viewCount: 1 } },
  { new: true, session },
);
